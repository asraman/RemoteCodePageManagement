///////////////////////////////////////////////////////////////////////////
// DependencyAnalyzer.h : implements dependency analysis                 //
// ver 1.0                                                               //
//                                                                       // 
...
std::map<std::string, std::vector<std::string>> DependencyAnalysis::summonParser(std::vector<std::string> files)
{
	std::string fileSpec;
	...
	for (size_t i = 0; i < files.size(); ++i)  // iterate over files
	{
		...
		try
		{
			...
			DepTable(files, pGlobalScope, fileSpec,dependencyMap);
...
void DependencyAnalysis::analyzeDepT(std::map<std::string, std::vector<std::string>>& depTmap)
{
	if (depTmap.size() == 0)
		return;
...
void DependencyAnalysis::DepTable(std::vector<std::string> files, CodeAnalysis::ASTNode * pGlobalScope, std::string fileSpec, std::map<std::string, std::vector<std::string>>& dependencyMap)
{
...
void DependencyAnalysis::newInfoTable(CodeAnalysis::ASTNode * pGlobalScope, std::string file)
{
...
DependencyAnalysis::TypeInfo DependencyAnalysis::findTypeInfo(CodeAnalysis::ASTNode * pGlobalScope)
{
